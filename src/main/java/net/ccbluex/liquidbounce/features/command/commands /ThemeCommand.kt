 ...
 * https://github.com/ZNmRtClient/FDPClient/
 */
...
class ThemeCommand : Command("theme", arrayOf("thememanager", "tm", "themes")) {
        ...
        if (args.size > 1) {
            when {
                args[1].equals("load", ignoreCase = true) -> {
                    if (args.size > 2) {
                        val themeFile = File(SilenceClient.fileManager.themesDir, args[2])
                        ...
                        if (themeFile.exists()) {
                            try {
                                ...
                                SilenceClient.hud = Config(theme).toHUD()
                                SilenceClient.isStarting = false
                                chat("§6Theme applied successfully.")
                                SilenceClient.hud.addNotification(Notification("theme", "Updated HUD Theme.", NotifyType.SUCCESS))
                                playEdit()
                            } catch (e: IOException) {
                                e.printStackTrace()
                            }
                ...
                args[1].equals("save", ignoreCase = true) -> {
                    if (args.size > 2) {
                        ...
                        try {
                            ...
                            val settingsTheme = Config(SilenceClient.hud).toJson()
                            chat("§9Saving theme...")
    ...
    private fun getLocalThemes(): Array<File>? = SilenceClient.fileManager.themesDir.listFiles()
}
