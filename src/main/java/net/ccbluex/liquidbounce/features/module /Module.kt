...
open class Module @JvmOverloads constructor(
    ...
    var array = array
        set(new) {
            ...
            if (!SilenceClient.isStarting) {
                SilenceClient.configManager.smartSave()
            }
        }
        ...
        set(value) {
            ...
            if (!SilenceClient.isStarting) {
                if (value) {
                    ...
                    SilenceClient.hud.addNotification(Notification(LanguageManager.getAndFormat("notify.module.title"), LanguageManager.getAndFormat("notify.module.enable", localizedName), NotifyType.SUCCESS))
                } else {
                    SoundModule.playSound(false)
                    SilenceClient.hud.addNotification(Notification(LanguageManager.getAndFormat("notify.module.title"), LanguageManager.getAndFormat("notify.module.disable", localizedName), NotifyType.ERROR))
                }
            }
