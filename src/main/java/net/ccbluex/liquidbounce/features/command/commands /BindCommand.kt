...
class BindCommand : Command("bind", emptyArray()) {
    ...
    override fun execute(args: Array<String>) {
        if (args.size > 1) {
            // Get module by name
            val module = SilenceClient.moduleManager.getModule(args[1])
            ...
            if (args.size > 2) {
                ...
                SilenceClient.hud.addNotification(
                    Notification("KeyBind", "Bound ${module.name} to ${Keyboard.getKeyName(key)}.", NotifyType.INFO)
                )
                playEdit()
            } else {
                SilenceClient.moduleManager.pendingBindModule = module
    ...
    override fun tabComplete(args: Array<String>): List<String> {
        ...
        return when (args.size) {
            1 -> SilenceClient.moduleManager.modules
                    .map { it.name }
