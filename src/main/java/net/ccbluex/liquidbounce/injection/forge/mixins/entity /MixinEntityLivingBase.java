...
public abstract class MixinEntityLivingBase extends MixinEntity {
    ...
    private void headLiving(CallbackInfo callbackInfo) {
        if (SilenceClient.moduleManager.getModule(NoJumpDelay.class).getState())
            jumpTicks = 0;
    }

    @Inject(method = "onLivingUpdate", at = @At(value = "FIELD", target = "Lnet/minecraft/entity/EntityLivingBase;isJumping:Z", ordinal = 1))
    private void onJumpSection(CallbackInfo callbackInfo) {
        final Jesus jesus = SilenceClient.moduleManager.getModule(Jesus.class);
    ...
    private double ViaVersion_MovementThreshold(double constant) {
        if (Objects.requireNonNull(SilenceClient.moduleManager.getModule(ViaVersionFix.class)).getState())
            return 0.003D;
        return 0.005D;
    }
    ...
    private void isPotionActive(Potion p_isPotionActive_1_, final CallbackInfoReturnable<Boolean> callbackInfoReturnable) {
        final AntiBlind antiBlind = SilenceClient.moduleManager.getModule(AntiBlind.class);
