...
class FriendCommand : Command("friend", arrayOf("friends")) {
    ...
    override fun execute(args: Array<String>) {
        if (args.size > 1) {
            val friendsConfig = SilenceClient.fileManager.friendsConfig
                ...
                args[1].equals("add", ignoreCase = true) -> {
                    if (args.size > 2) {
                        ...
                        if (if (args.size > 3) friendsConfig.addFriend(name, StringUtils.toCompleteString(args, 3)) else friendsConfig.addFriend(name)) {
                            SilenceClient.fileManager.saveConfig(friendsConfig)
        ...
        return when (args.size) {
            ...
            2 -> {
                when (args[0].lowercase()) {
                    ...
                    "remove" -> {
                        return SilenceClient.fileManager.friendsConfig.friends
                                .map { it.playerName }
                                .filter { it.startsWith(args[1], true) }
                    }
                }
