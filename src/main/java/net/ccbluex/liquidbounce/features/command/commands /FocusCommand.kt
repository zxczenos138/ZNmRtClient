    ...
    override fun execute(args: Array<String>) {
        if (args.size == 3) {
            ...
            when (focused.lowercase()) {
                "add" -> {
                    SilenceClient.combatManager.focusedPlayerList.add(entity)
                    alert("Successfully added §a${target.lowercase()}§3 into the focus list.")
                    return
                }
                "remove" -> {
                    if (SilenceClient.combatManager.focusedPlayerList.contains(entity)) {
                        SilenceClient.combatManager.focusedPlayerList.remove(entity)
            ...
            SilenceClient.combatManager.focusedPlayerList.clear()
