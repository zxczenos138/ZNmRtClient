...
class FileManager : MinecraftInstance() {
    val dir = File(mc.mcDataDir, "SILENCE-CLIENT")
    ...
    private fun saveConfig(config: FileConfig, ignoreStarting: Boolean) {
        if (!ignoreStarting && SilenceClient.isStarting) return
        try {
    ...
    fun loadBackground() {
        if (backgroundFile.exists()) {
            try {
                ...
                SilenceClient.background = ResourceLocation(SilenceClient.CLIENT_NAME.lowercase() + "/background.png")
                mc.textureManager.loadTexture(SilenceClient.background, DynamicTexture(bufferedImage))
        ...
        if (modulesFile.exists()) {
            ...
            try {
                ...
                for ((key, value) in jsonElement.asJsonObject.entrySet()) {
                    val module = SilenceClient.moduleManager.getModule(key)
                    if (module != null) {
        ...
        if (macrosFile.exists()) {
            ...
            try {
                ...
                for (jsonElement in jsonArray) {
                    ...
                    SilenceClient.macroManager.macros
