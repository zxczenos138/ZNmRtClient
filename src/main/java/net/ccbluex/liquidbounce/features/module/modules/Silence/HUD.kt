...
object HUD : Module("HUD", category = ModuleCategory.CLIENT, array = false, defaultOn = true) {
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        SilenceClient.hud.render(false, event.partialTicks)
    ...
    fun onScreen(event: ScreenEvent) {
        ...
        } else if (mc.entityRenderer.shaderGroup != null && mc.entityRenderer.shaderGroup!!.shaderGroupName.contains("silenceclient/blur.json")) {
    ...
    fun onKey(event: KeyEvent) {
        SilenceClient.hud.handleKey('a', event.key)
    }
