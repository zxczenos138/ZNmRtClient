...
class ConfigManager {
    ...
    var configFile = File(SilenceClient.fileManager.configsDir, "$nowConfig.json")
    var needSave = false
    ...
    fun load(name: String, save: Boolean = true) {
        ...
        nowConfig = name
        configFile = File(SilenceClient.fileManager.configsDir, "$nowConfig.json")
    ...
    fun save(saveConfigSet: Boolean = nowConfigInFile != nowConfig, forceSave: Boolean = false) {
        if (SilenceClient.isLoadingConfig && !forceSave) {
            return
        }
    ...
    fun loadLegacySupport() {
        if (FDPClient.fileManager.loadLegacy()) {
            if (File(SilenceClient.fileManager.configsDir, "$nowConfig.json").exists()) {
                nowConfig = "legacy"
            ...
            script.lines().filter { it.isNotEmpty() && !it.startsWith('#') }.forEachIndexed { _, s ->
                ...
                when (args[0]) {
                    ...
                    "targetDead" -> {
                        ...
                        val value = args[2]
                        val module = SilenceClient.moduleManager.getModule(moduleName) ?: return@forEachIndexed
        ...
        val oldSettingDir = File(SilenceClient.fileManager.dir, "settings")
        if (oldSettingDir.exists()) {
            oldSettingDir.listFiles().forEach {
                ...
                if (!SilenceClient.fileManager.legacySettingsDir.exists()) {
                    SilenceClient.fileManager.legacySettingsDir.mkdir()
                }
